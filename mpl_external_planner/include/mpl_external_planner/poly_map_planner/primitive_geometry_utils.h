...
template <int Dim>
bool collide(const Primitive<Dim>& pr, const PolyhedronLinearObstacle<Dim>& obs, decimal_t t) {
  vec_E<Vec6f> cs(Dim);
	for(int i = 0; i < Dim; i++)
		cs[i] = pr.pr(i).coeff();
...
template <int Dim>
bool collide(const Primitive<Dim>& pr, const PolyhedronNonlinearObstacle<Dim>& obs, decimal_t t) {
  vec_E<Vec6f> cs(Dim);
	for(int i = 0; i < Dim; i++)
		cs[i] = pr.pr(i).coeff();
